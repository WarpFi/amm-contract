...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    constructor(IWETH _CAKE, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        CAKE = _CAKE;
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        ...
        CAKE.transferFrom(receiver, address(this), _amount);
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            // CAKE.transferFrom(address(this), _to, _amount);
            CAKE.withdraw(_amount);
            safeTransferETH(_to, _amount);
        }
    }

    receive() external payable {
        if (msg.sender != address(nonfungiblePositionManager) && msg.sender != WETH && msg.sender != address(CAKE)) revert();
    }
}
